...
function MoodBoardLayout({ selectedProducts, presentation }) {
  ...
  // Masonry-like columns with varied sizes
...
function RoomConceptLayout({ selectedProducts, presentation }) {
  return (
    <div className="space-y-6">
      ...
      {/* Products with placement suggestions */}
...
export default function PresentationBuilder() {
  const [allProducts] = useState(() => getCompareItems());
  const [selectedIds, setSelectedIds] = useState(() =>
    new Set(getCompareItems().map((p) => p.id))
  );
